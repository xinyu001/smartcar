...
uint8 imgbuff[CAMERA_SIZE];                       //定义存储接收图像的数组  CAMERA_SIZE是图像大小的字节数（80*60/8）
...
//    {OV7725_DSP_Ctrl4    , 0x00},
    ...
    {OV7725_COM3         , 0xD0},
    {OV7725_COM8         , 0xFF},
    {OV7725_AEW          , 0x40},  //自动曝光参数
    {OV7725_AEB          , 0x15},  //自动曝光参数
    {OV7725_VPT          , 0x32},
    {OV7725_SDE          , 0X24},
    {OV7725_BRIGHT       , 0x20},  //亮度增益，越大越亮
    {OV7725_SIGN         , 0x06},
...
//    {OV7725_UVADJ1       , 0x02},
    ...
    {OV7725_CNST         , 0x45},  // 0x40  0100 0000=64  越低越容易变黑
    {OV7725_UVADJ0       , 0x11},
    {OV7725_UVADJ1       , 0x02},
...
void camera_port_init()
{
    ...
    dma_portx2buff_init(DMA_CH0, (void *)&PTD_B0_IN, imgbuff, PTC19, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
    ...
    //port_init(PTC19, ALT1 | DMA_RISING | PULLUP );         //PCLK
    //port_init(PTD8, ALT1 | IRQ_RISING | PULLUP | PF);     //场中断，上拉，上降沿触发中断，带滤波
    
    port_init(PTC19, ALT1 | DMA_FALLING | PULLDOWN );         //PCLK
    port_init(PTD8, ALT1 | IRQ_FALLING | PULLDOWN | PF);     //场中断，上拉，上降沿触发中断，带滤波
    
    enable_irq(PORTD_IRQn);                        //关闭PTC的中断
}
