...
const uint8 mykc_logo[1024]=
{
    ...
    /*--  调入了一幅图像：C:\Users\Lenovo\Desktop\捕获.bmp  --*/
    /*--  宽度x高度=128x64  --*/
    0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//1位表示1个像素
    0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//1个数值表示8个像素
    0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,//16列64行，共1024个数组元素
...
const uint8 F6x8[][6] =
{
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },   // sp   /*一行表示一个字符*/
    { 0x00, 0x00, 0x00, 0x2f, 0x00, 0x00 },   // !   /*每个字符用6列8行像素表示*/
    { 0x00, 0x00, 0x07, 0x00, 0x07, 0x00 },   // "   /*每个数组元素(8bit)表示一列像素，低位在上*/
    ...
    { 0x14, 0x14, 0x14, 0x14, 0x14, 0x14 },   // horiz lines（地平线）
    { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF }    // cursor（光标）
};
...
void OLED_WrLogo(uint8 data) //画logo专用函数
{
       uint8 i=8;	
       DC_HIGH;
       asm("nop"); /*asm(“nop”)是内嵌汇编做空指令延时用的，其中asm()的作用是指函数内部的参数翻译为汇编指令，其作用是在C语言环境下直接使用汇编指令执行。 */
       asm("nop"); /*nop是一个空等待汇编指令，这个指令执行时，单片机是什么也不做，仅仅起一个时间延时作用。 */
       D0_LOW;
       asm("nop");
       asm("nop"); 
  while(i--)
