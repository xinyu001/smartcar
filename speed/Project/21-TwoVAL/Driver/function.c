...
void Para_Init()
{
  Speed_H=0.75;  //0.8  0.7
  Speed_M=0.65;  //0.62  0.7
  Speed_L=0.52;//0.42;
  if(Style==0)
    SetSpeed=0.25;//0.9  原来0.6
  if(Style==1)
    SetSpeed=0.5; //0.6//s
...
void getadval()
{
  ...
  AD_val_1 = adc_ave(ADC1_SE8, ADC_16bit,10);    //最左边电感电压   
  AD_val_2 = adc_ave(ADC1_SE9, ADC_16bit,10);   //最右边电感电压  
  AD_val_3 = adc_ave(ADC1_SE10, ADC_16bit,10);    //电感电压 
